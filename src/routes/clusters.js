...
// Disconnect from cluster (admin only)
router.post('/:clusterId/disconnect', authenticateToken, isAdmin, (req, res) => {
    ...
    try {
        // Get cluster manager from task manager
        if (taskManager && taskManager.clusterManager) {
            taskManager.clusterManager.disconnectFromCluster(clusterId);
            res.json({ message: 'Cluster disconnected successfully' });
        } else {
            res.status(500).json({ error: 'Cluster manager not available' });
        }
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// Reconnect to cluster (admin only)
router.post('/:clusterId/reconnect', authenticateToken, isAdmin, (req, res) => {
    const { clusterId } = req.params;
    ...
    try {
        ...
        if (taskManager && taskManager.clusterManager) {
            taskManager.clusterManager.connectToCluster(cluster);
            res.json({ message: 'Cluster reconnected successfully' });
        } else {
            res.status(500).json({ error: 'Cluster manager not available' });
        }
    } catch (error) {
